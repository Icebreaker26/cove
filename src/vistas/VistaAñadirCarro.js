...
const VistaAñadirCarro = () =>{
    ...
    const handleEnviar = async () => {
        ...
        if (campos.every(campo => campo)) {
          ...
          // Actualizar las ventas localmente
          cliente.ventas.push(solicitudVenta);
          console.log(cliente.ventas);
      
          try {
            // Realizar la solicitud PUT para actualizar el cliente en el servidor
            const response = await axios.put(`http://localhost:8000/users/${cliente._id}`, {
              ventas: cliente.ventas,
            });
      
            // Si la respuesta es exitosa, actualizar el estado local
            setCLIENTE((prevCLIENTE) =>
              prevCLIENTE.map((c) => (c._id === cliente._id ? response.data : c))
            );
      
            console.log("Venta enviada correctamente:", response.data);
      
            // Llamar a la función para cerrar o abrir el perfil, según sea necesario
            
            abrirPerfil();
          } catch (error) {
            console.error("Error al enviar la venta:", error.response?.data || error.message);
          }
        } else {
          console.log("Faltan campos para completar la venta");
        }
      };
